...
export const ZSimpleTriggerJobOptionsSchema = z.object({
  ...
  payload: z
    .unknown()
    .refine((x) => x !== undefined, { message: "payload is required." }),
  timestamp: z.number().optional(),
});

export type TriggerJobOptions<
  Definitions extends ReadonlyArray<JobDefinition> = []
> = {
  [K in keyof Definitions]: {
    id?: string;
    name: Definitions[K]["trigger"]["name"];
    payload: Definitions[K]["trigger"]["schema"] extends z.ZodType<infer Shape>
      ? Shape
      : unknown;
    timestamp?: number;
  };
